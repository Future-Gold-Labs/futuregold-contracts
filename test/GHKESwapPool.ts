...
describe("GHKESwapPool", async () => {
  ...
  it("swap", async () => {
    ...
    const offchainPrice = BigInt((offchainPriceData.client_buy + offchainPriceData.water_level_buy) * 1e18); // chainlink 预言机的价格精度都是 8 位，但在链上都处理成了 18 位所以这里要乘以 1e18 转换为 18 位精度
